...
namespace AdaptiveNamespace::XamlHelpers
{
    ...
    HRESULT WrapInTouchTarget(_In_ IAdaptiveCardElement* adaptiveCardElement,
                              _In_ IUIElement* elementToWrap,
                              _In_ IAdaptiveActionElement* action,
                              _In_ IAdaptiveRenderContext* renderContext,
                              bool fullWidth,
                              const std::wstring& style,
                              _COM_Outptr_ IUIElement** finalElement) noexcept
    {
        ...
        RETURN_IF_FAILED(button.As(&buttonAsContentControl));
        RETURN_IF_FAILED(buttonAsContentControl->put_Content(elementToWrap));
        ...
        if (fullWidth)
        {
            RETURN_IF_FAILED(spacingConfig->get_Padding(&cardPadding));
        }

        ComPtr<IFrameworkElement> buttonAsFrameworkElement;
        RETURN_IF_FAILED(button.As(&buttonAsFrameworkElement));
        ...
        if (adaptiveCardElement != nullptr)
        {
            ...
            RETURN_IF_FAILED(adaptiveCardElement->get_Spacing(&elementSpacing));
            UINT spacingSize;
            RETURN_IF_FAILED(GetSpacingSizeFromSpacing(hostConfig.Get(), elementSpacing, &spacingSize));
            ...
            RETURN_IF_FAILED(button.As(&buttonAsControl));
            RETURN_IF_FAILED(buttonAsControl->put_Padding({(double)cardPadding, topBottomPadding, (double)cardPadding, topBottomPadding}));

            double negativeCardMargin = cardPadding * -1.0;
            double negativeTopBottomMargin = topBottomPadding * -1.0;

            RETURN_IF_FAILED(buttonAsFrameworkElement->put_Margin(
        ...
        if (action != nullptr)
        {
            ...
            RETURN_IF_FAILED(button.As(&buttonAsDependencyObject));
            ...
            RETURN_IF_FAILED(automationPropertiesStatics->SetName(buttonAsDependencyObject.Get(), title.Get()));

            RETURN_IF_FAILED(WireButtonClickToAction(button.Get(), action, renderContext));
        }

        RETURN_IF_FAILED(button.CopyTo(finalElement));
        return S_OK;
    }

    HRESULT WireButtonClickToAction(_In_ IButton* button, _In_ IAdaptiveActionElement* action, _In_ IAdaptiveRenderContext* renderContext) noexcept
    {
        ...
        RETURN_IF_FAILED(localButton.As(&buttonBase));

        EventRegistrationToken clickToken;
        RETURN_IF_FAILED(buttonBase->add_Click(Callback<IRoutedEventHandler>([strongAction, actionInvoker](IInspectable* /*sender*/, IRoutedEventArgs *
                                                                                                           /*args*/) -> HRESULT {
                                                   RETURN_IF_FAILED(actionInvoker->SendActionEvent(strongAction.Get()));
                                                   return S_OK;
                                               }).Get(),
                                               &clickToken));
        return S_OK;
    ...
    HRESULT HandleSelectAction(_In_ IAdaptiveCardElement* adaptiveCardElement,
                               ...
                               _COM_Outptr_ IUIElement** outUiElement)
    {
        ...
        {
            RETURN_IF_FAILED(XamlHelpers::WrapInTouchTarget(
                adaptiveCardElement, uiElement, selectAction, renderContext, fullWidthTouchTarget, L"Adaptive.SelectAction", outUiElement));
        }
        else
        {
            ...
            RETURN_IF_FAILED(localUiElement.CopyTo(outUiElement));
        }
        return S_OK;
    }
